...
if command -v apt-get &> /dev/null; then
    apt-get update
    apt-get install -y build-essential libudev-dev libevdev-dev clang-format pkg-config libx11-dev libgtk-3-dev debhelper devscripts libatspi-dev || apt-get install -y libatspi2.0-dev
